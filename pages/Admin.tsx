...
import { Product, AdminTab, ProductSize } from '../types';
import { saveProduct, deleteProduct, saveProducts, resetData } from '../services/storage';
...
export const Admin: React.FC<AdminProps> = ({ products, refreshProducts }) => {
  ...
  const [timeRange, setTimeRange] = useState('30D'); // Mock time range state
  ...
  const [cost, setCost] = useState<string>(''); // Cost of Goods
  ...
  // Mock Sales Data (Dynamic based on total value scale)
  const salesData = useMemo(() => {
    const baseRevenue = totalValue * 0.15; // Simulate monthly revenue relative to stock value
    return [
      ...
      { month: 'Jun', revenue: baseRevenue * 1.4, cost: baseRevenue * 1.4 * 0.35 },
        ...
        ...item,
        profit: item.revenue - item.cost, // Calculated Profit
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        headers.forEach((header, index) => {
            if (values[index] !== undefined) {
                 ...
                 if (header === 'price' || header === 'stock' || header === 'cost') {
                    ...
                    if (val) {
                        try {
                            p[header] = val.split('|').map((s: string) => {
                                const [l, pr] = s.split(':');
                                return { label: l.trim(), price: parseFloat(pr) };
                            }).filter((s: any) => s.label && !isNaN(s.price));
                        } catch (e) {
                            p[header] = [];
                        }
                    } else {
        ...
        if (p.name && p.price) {
            productsToAdd.push({
                ...p,
                id: crypto.randomUUID(),
                imageUrl: p.imageUrl || 'https://picsum.photos/800/800',
                sizes: p.sizes || []
            });
        }
    }
    ...
    <div className="flex h-screen bg-[#F9F8F4]">
      ...
      <div className="w-64 bg-[#1A1816] text-[#E5E0D8] flex flex-col shadow-xl z-10">
        ...
        <div className="p-6 border-t border-[#2D2A26]">
          <button onClick={() => window.location.reload()} className="w-full flex items-center justify-center px-4 py-2 border border-[#2D2A26] hover:bg-[#2D2A26] transition-colors rounded text-sm">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                            <input 
                                className="bg-transparent text-xs border-b border-[#E5E0D8] focus:border-[#2D2A26] outline-none w-48 text-[#786B59]" 
                                placeholder="Keywords for AI (e.g. soft, durable)"
                                onChange={(e) => { /* Optional: update a dedicated keyword state if needed */ }}
                            />
                            <button 
                    ...
                    <tbody className="divide-y divide-[#E5E0D8]">
                      {products.map((product) => (
                        <tr key={product.id} className="hover:bg-[#F9F8F4] transition-colors group">
                          <td className="p-4">
                            <div className="flex items-center gap-3">
                              <img src={product.imageUrl} alt={product.name} className="w-10 h-10 object-cover bg-[#E5E0D8]" />
          ...
          {activeTab === 'SETTINGS' && (
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="bg-white p-8 rounded-none shadow-sm border border-[#E5E0D8]">
                <h3 className="text-lg font-medium text-[#2D2A26] mb-4">Data Management</h3>
                <p className="text-[#4A4036] mb-6 max-w-xl">
                    Resetting data will clear all local changes and revert the catalogue to the default initial collection. 
                    This cannot be undone.
                </p>
                <Button variant="danger" onClick={() => { if(window.confirm('Reset all data?')) resetData(); }}>
                  Reset to Defaults
                </Button>

                <div className="mt-8 pt-8 border-t border-[#E5E0D8]">
                   <h3 className="text-lg font-medium text-[#2D2A26] mb-4">Bulk Import Instructions</h3>
                   <div className="text-sm text-[#4A4036] space-y-2 font-mono bg-[#F9F8F4] p-4 border border-[#E5E0D8]">
                        <p><strong>Required Headers:</strong> name, category, description, price, cost, stock, imageUrl, sizes</p>
                        <p><strong>Sizes Format:</strong> Label:Price|Label:Price</p>
                        <p><strong>Example:</strong> "Print","Art","Desc...",45,15,10,"url","8x10:45|11x14:65"</p>
                   </div>
                </div>
              </div>
            </div>
